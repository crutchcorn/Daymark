...
import app.drawmark.android.lib.textcanvas.DefaultCursorThickness
import app.drawmark.android.lib.textcanvas.Handle
import app.drawmark.android.lib.textcanvas.MenuItemsAvailability
...
import app.drawmark.android.lib.textcanvas.getLineHeight
import app.drawmark.android.lib.textcanvas.input.TextFieldCharSequence
import app.drawmark.android.lib.textcanvas.input.getSelectedText
import app.drawmark.android.lib.textcanvas.input.internal.IndexTransformationType.Deletion
import app.drawmark.android.lib.textcanvas.input.internal.IndexTransformationType.Insertion
import app.drawmark.android.lib.textcanvas.input.internal.IndexTransformationType.Replacement
import app.drawmark.android.lib.textcanvas.input.internal.IndexTransformationType.Untransformed
import app.drawmark.android.lib.textcanvas.input.internal.SelectionWedgeAffinity
import app.drawmark.android.lib.textcanvas.input.internal.TextFieldDecoratorModifierNode
import app.drawmark.android.lib.textcanvas.input.internal.TextLayoutState
import app.drawmark.android.lib.textcanvas.input.internal.TransformedTextFieldState
import app.drawmark.android.lib.textcanvas.input.internal.WedgeAffinity
import app.drawmark.android.lib.textcanvas.input.internal.coerceIn
import app.drawmark.android.lib.textcanvas.input.internal.findClosestRect
import app.drawmark.android.lib.textcanvas.input.internal.fromDecorationToTextLayout
import app.drawmark.android.lib.textcanvas.input.internal.fromTextLayoutToDecoration
import app.drawmark.android.lib.textcanvas.input.internal.getIndexTransformationType
import app.drawmark.android.lib.textcanvas.input.internal.selection.TextToolbarState.Cursor
import app.drawmark.android.lib.textcanvas.input.internal.selection.TextToolbarState.None
import app.drawmark.android.lib.textcanvas.input.internal.selection.TextToolbarState.Selection
...
import app.drawmark.android.lib.textcanvas.selection.PlatformSelectionBehaviors
import app.drawmark.android.lib.textcanvas.selection.SelectionAdjustment
import app.drawmark.android.lib.textcanvas.selection.SelectionLayout
import app.drawmark.android.lib.textcanvas.selection.awaitSelectionGestures
import app.drawmark.android.lib.textcanvas.selection.containsInclusive
import app.drawmark.android.lib.textcanvas.selection.getAdjustedCoordinates
import app.drawmark.android.lib.textcanvas.selection.getSelectionHandleCoordinates
import app.drawmark.android.lib.textcanvas.selection.getTextFieldSelectionLayout
import app.drawmark.android.lib.textcanvas.selection.isMouseOrTouchPad
import app.drawmark.android.lib.textcanvas.selection.visibleBounds
...
/**
 * Creates a menu item action callback for a [TextFieldSelectionState].
 *
 * This function generates a lambda that performs a specified [operation] when an item is clicked.
 * The lambda also updates the [TextToolbarState] to the given [desiredState] after the operation is
 * executed. If the [enabled] parameter is `false`, the function returns `null`, indicating that the
 * menu item is disabled and no action will be taken.
 *
 * @param enabled A Boolean indicating whether the menu item is enabled. If `false`, the function
 *   returns `null`, disabling the menu item.
 * @param desiredState The desired [TextToolbarState] to be set after an [operation] is performed.
 * @param operation The action to be executed when the menu item is clicked.
 * @return A lambda that performs the specified [operation] and updates the [TextToolbarState], or
 *   `null` if [enabled] is `false`.
 * @receiver The [TextFieldSelectionState] representing the current selection state of a text field
 *   for which the menu item is being created.
 */
...
internal interface TextToolbarHandler {
    ...
    /**
     * @param selectionState - [TextFieldSelectionState] of the text field for which the
     *   [TextToolbar] is requested
     * @param rect - the rectangle where the text field is positioned (content rectangle). It's used
     *   to show the [TextToolbar] in the proper place.
     */
